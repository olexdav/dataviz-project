...
knitr::opts_knit$set(root.dir = '/cloud/project/dataviz-project')
```
...
head(pets)
glimpse(pets)
# amount of unique pet names
length(unique(pets$animal_s_name))
# summarize pets by species
df <- pets %>% group_by(species)
df %>% summarize(N = n())
```
...
### Explore the data
# 5 most common cat and dog breeds
...
glimpse(steak)
summary(steak)
str(steak$Do.you.eat.steak.)
#ggplot(data = steak) +
#  geom_bar(aes(x = Do.you.eat.steak.))
steak %>%
